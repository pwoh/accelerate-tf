...
--import qualified Data.Storable as S -- how to use this one?
import Test.HUnit ((@=?))
...
-- Doesn't work - not a matrix?
--derp2 :: IO (Float)
--derp2 = TF.runSession $ do
...
--    wz <- TF.assign vz (wx `TF.matMul` wy)
--    TF.Scalar result <- TF.run (wz)
--    return result
